...
import {
  ...
  processDocumentExampleSchema,
  processFaqSchema,
  processFeeSchema,
  processInstructionSchema,
  processVideoTutorialSchema,
  processStatusSchema,
...
export const adminGuideFormSchema = z.object({
  ...
  video_tutorials: z.array(processVideoTutorialSchema).optional(),
  document_examples: z.array(processDocumentExampleSchema).optional(),
  plain_english_summary: z.string().trim().max(700).nullable(),
  prepare_first: z.array(z.string().trim().min(1).max(180)).optional(),
  common_confusions: z.array(z.string().trim().min(1).max(220)).optional(),
...
export function parseAdminGuideForm(formData: FormData) {
  ...
  return adminGuideFormSchema.parse({
    ...
    video_tutorials: parseJsonField(formData.get("video_tutorials"), z.array(processVideoTutorialSchema)),
    document_examples: parseJsonField(formData.get("document_examples"), z.array(processDocumentExampleSchema)),
    plain_english_summary: getOptionalString(formData, "plain_english_summary"),
    prepare_first: parseJsonField(formData.get("prepare_first"), z.array(z.string().trim().min(1).max(180))),
    common_confusions: parseJsonField(formData.get("common_confusions"), z.array(z.string().trim().min(1).max(220))),
